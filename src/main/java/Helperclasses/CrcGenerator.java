...
public class CrcGenerator {
	...
	public static byte[] calculateCrc(byte[] bytes) {
		if (bytes == null)
			bytes = new byte[] { (byte) 0xff, (byte) 0x11, (byte) 0x22, (byte) 0x33, (byte) 0x44, (byte) 0x55,
					(byte) 0x66, (byte) 0x77, (byte) 0x88, (byte) 0x99, (byte) 0xaa, (byte) 0xbb, (byte) 0xcc,
		...
		for (counter = 0; counter < bytes.length; counter++) {
			...
			// short byte2 = (short) (((crc >>> 8)) & 0xff);

			short data = (short) (((crc >> 8) ^ bytes[counter]) & 0x00FF);

			crc = (short) ((short) (crc_table[data] ^ (short) (crc << 8)) & (byte) 0xFF);

			/*
			 * crc = (crc<<8) ^ crc_table[((crc>>8) ^100)&0x00FF];
			 * 
...
//		Logger.i(TAG, "calculateCrc:" + crc + ":" + Integer.toHexString(b) + ":" + b + ":"
//				+ (byte) (array[0] & (byte) 0xFF) + ":" + (byte) (array[1] & (byte) 0xFF));
